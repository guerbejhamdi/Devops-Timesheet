...
import tn.esprit.spring.entities.Missions;
...
public class RestControlTimesheet {
	...
	@ResponseBody
	public int ajouterMission(@RequestBody Missions mission) {
		Mission persistantMission= new Mission();
		persistantMission.setName(mission.getName());
		persistantMission.setDescription(mission.getDescription());
		itimesheetservice.ajouterMission(persistantMission);
		return persistantMission.getId();
	}
